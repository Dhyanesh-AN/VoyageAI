...
DEFAULT_ORIGIN_IATA = os.getenv("DEFAULT_ORIGIN_IATA", "LHR")
...
def parse_route(query: str):
    ...
    match_from = re.search(r"\bfrom\s+(.+?)(?:\s|$)", q_lower)
    
    if match_from:
        origin_text = match_from.group(1)
        dep_iata = resolve_location_to_iata(origin_text)
        
        # Remove the 'from X' part and check if another location exists
        remaining_text = q_lower.replace(match_from.group(0), "")
        mentions = find_location_mentions(remaining_text)
        
        arr_iata = resolve_location_to_iata(mentions[0]) if mentions else None
        return dep_iata, arr_iata

    # Pattern: flights to X
    match_to = re.search(r"\bto\s+(.+?)(?:\s|$)", q_lower)

    if match_to:
        dest_text = match_to.group(1)
        arr_iata = resolve_location_to_iata(dest_text)
        
        remaining_text = q_lower.replace(match_to.group(0), "")
        mentions = find_location_mentions(remaining_text)
        
        dep_iata = resolve_location_to_iata(mentions[0]) if mentions else None
        return dep_iata, arr_iata
