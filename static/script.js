...
// Fixed pipeline order — matches the actual agent execution order in the
// backend graph, so numbering on the departures board means something real.
...
function showWorkflow(data) {
  ...
  const orderedSelected = AGENT_PIPELINE_ORDER.filter((agent) => selected.includes(agent));

  orderedSelected.forEach((agent, i) => {
    const row = document.createElement("div");
    row.className = "agent-row";
    row.style.animationDelay = `${i * 90}ms`;
    row.innerHTML = `
      <span class="row-index">${String(i + 1).padStart(2, "0")}</span>
      <span class="row-name">${AGENT_LABELS[agent] || agent}</span>
      <span class="row-status"><span class="dot"></span>Complete</span>
    `;
  ...
  renderTripStatus(data);
}

function renderTripStatus(data) {
  const section = document.getElementById("tripStatusSection");
  const hasBudgetData = !!data.budget_risk_level;
  ...
  const feasibleText = document.getElementById("feasibleText");
  const replannedText = document.getElementById("replannedText");
  const roundsText = document.getElementById("roundsText");

  const risk = (data.budget_risk_level || "low").toLowerCase();
  riskDot.className = `ts-dot risk-${risk}`;
  riskText.textContent = risk.charAt(0).toUpperCase() + risk.slice(1);

  feasibleText.textContent = data.budget_feasible === false ? "No" : "Yes";
  replannedText.textContent = data.replanned ? "Yes" : "No";
  roundsText.textContent = data.negotiation_rounds ?? 0;
...
function showApproval(data) {
  ...
  approvalRequest.textContent = data.approval_request ||
    "Approve the draft or leave feedback and the itinerary agent will revise it.";
  section.classList.remove("hidden");
}
